...
const main = module.exports = (searchString, options, callback) => { // eslint-disable-line consistent-return
  // Check whether options were provided
  ...
  if (!options.gl) options.gl = 'US';
  if (!options.hl) options.hl = 'en';
  UTIL.getPage(currentRef ? UTIL.buildFromNextpage(currentRef, options.gl, options.hl) : UTIL.buildLink(searchString, options.gl, options.hl), requestOptions, (err, body) => { // eslint-disable-line consistent-return, max-len
...
const getFilters = main.getFilters = (searchString, options, callback) => { // eslint-disable-line consistent-return
  // Check whether options were provided
  if (typeof options === 'function') {
    callback = options;
    options = { };
  ...
  if (!callback) {
    return new Promise((resolve, reject) => {
      getFilters(searchString, options, (err, info) => err ? reject(err) : resolve(info));
    });
  }
  ...
  if (parsedQuery.query.sp && parsedQuery.query.search_query) {
    queryString = UTIL.buildFromNextpage(searchString, options.gl, options.hl);
  } else {
    queryString = UTIL.buildLink(searchString, options.gl, options.hl);
  }
