...
const BASE_SEARCH_URL = 'https://www.youtube.com/results?app=desktop';
...
const main = module.exports = async(searchString, options, rt = 3) => {
  ...
  // let body = await MINIGET(ref, opts.requestOptions).text()
  // body = body.replace(/\\x([0-9A-Fa-f]{2,4})/g, function() {
  //   return String.fromCharCode(parseInt(arguments[1], 16));
  // });
  body = await fetch(`https://www.youtube.com/results?app=desktop&sp=${options.sp}&gl=US&hl=en&search_query=${searchString}`).then(res => res.text())
  const parsed = UTIL.parseBody(body, opts);
  console.tron.log('parsed', parsed, body);
...
main.getFilters = async(searchString, options) => {
  ...
  const ref = BASE_SEARCH_URL + QS.encode(opts.query);
  const body = await fetch(`https://www.youtube.com/results?app=desktop&gl=US&hl=en&search_query=${searchString}`).then(res => res.text())
  const parsed = UTIL.parseBody(body, opts);
