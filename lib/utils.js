  ...
  for (const filterGroup of filterWrapper) {
    ...
    for (const filter of filterGroup.searchFilterGroupRenderer.filters) {
      ...
      const parsedFilter = {
        ...
        active: isSet,
        url: isSet ? null : URL.resolve(BASE_URL, targetURL),
...
exports.checkArgs = (searchString, options = {}) => {
  ...
  if (searchString.startsWith(BASE_URL)) {
    ...
    if (!obj.query.search_query) throw new Error('link has to include a "search_string" query');
  } else {
  ...
  if (options && options.sp) obj.query.sp = options.sp;
  return obj;
};
